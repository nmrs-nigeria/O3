...
class NMRSFormConverter:
    def __init__(self, root):
        ...
        self.forms_listbox.grid(row=1, column=0, columnspan=2, sticky="nsew")
    ...
    def generate_concepts_excel(self):
        ...
        ws.title = "Concepts"
        headers = [
            "Concept", "2.x Question with Concepts", "2.x Question text", "3.x Question (Concept) UUID",
            "Answer", "2.x Answers with their Concepts", "2.x Answer text", "2.x Answer Type", "Answer UUID"
        ]
        ws.append(headers)

        is_first_answer = True
        # 4. Iterate through each question (<obs> tag) and populate rows
        for obs in obs_tags:
            ...
            if self.concept_datatypes.get(q_cid) == 'coded' and not answer_concept_ids:
                ...
                for ans in db_answers:
                    # Find the concept_id for the answer's uuid
                    ans_cid = "N/A"
                    # This reverse lookup can be slow. We'll proceed with what we have.
                    # A better approach would be to build a reverse map if needed, but for now this is okay.
                    
                    q_data = ["Concept", q_cid, q_name, q_uuid] if is_first_answer else ["", "", "", ""]
                    ans_data = ["Answer", ans_cid, ans.get("label"), "Coded", ans.get("uuid")]
            ...
            if not answer_concept_ids and self.concept_datatypes.get(q_cid) != 'coded':
                ...
                # Only add if it's the first (and only) line for this question
                if is_first_answer:
                    ws.append(["Concept", q_cid, q_name, q_uuid, "", "", "", q_datatype.capitalize(), ""])
                    is_first_answer = False

            # Process the collected answer IDs
            else:
                for a_cid in answer_concept_ids:
                    ...
                    q_data = ["Concept", q_cid, q_name, q_uuid] if is_first_answer else ["", "", "", ""]
                    ws.append(q_data + ["Answer", a_cid, a_name, a_datatype.capitalize(), a_uuid])
                    is_first_answer = False
